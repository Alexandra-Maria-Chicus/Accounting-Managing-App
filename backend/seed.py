...
from app.db.models import Company, ContactPerson, Record, Role, Permission, RolePermission, User
...
def seed(db):
    ...
    if db.query(Record).count() > 0:
        print("Already seeded, skipping records.")
    else:
    ...
    if db.query(Role).count() > 0:
        print("Already seeded, skipping roles.")
    else:
        ...
        for pname in ["read_records", "write_records", "delete_records", "manage_companies"]:
            db.add(RolePermission(role_id=role_map["admin"], permission_id=perm_map[pname]))
        for pname in ["read_records", "write_records"]:
            db.add(RolePermission(role_id=role_map["employee"], permission_id=perm_map[pname]))
        db.add(RolePermission(role_id=role_map["client"], permission_id=perm_map["view_own_company"]))
        db.commit()
        print("Roles and permissions seeded.")

    # ── Users ─────────────────────────────────────────────────────────────────
    if db.query(User).count() > 0:
        print("Already seeded, skipping users.")
    else:
        role_map = {r.name: r.id for r in db.query(Role).all()}
        company_map = {c.name: c.id for c in db.query(Company).all()}
