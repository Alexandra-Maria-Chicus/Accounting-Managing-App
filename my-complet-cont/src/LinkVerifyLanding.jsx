...
export default function LinkVerifyLanding({ onLoginSuccess, onGoToLogin }) {
  ...
  const handleComplete = async () => {
    ...
    setError('');
    try {
      const user = await validate2FALink(token);
      storeUser(user);
    ...
    <div className="min-vh-100 d-flex align-items-center justify-content-center" style={{ backgroundColor: '#f8f9fa' }}>
      <Container style={{ maxWidth: '440px' }}>
        <Card className="border-0 shadow-sm rounded-4 p-4 text-center">
          ...
          <p className="text-muted small mb-4">Click the button below to complete your sign in.</p>
          ...
          {!error && (
            <Button
              ...
              disabled={loading}
            >
              {loading ? <Spinner size="sm" /> : 'Complete Sign In'}
            </Button>
          )}
